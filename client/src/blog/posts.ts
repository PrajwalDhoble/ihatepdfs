...
export const BLOG_POSTS: BlogPost[] = [
  ...
  {
    slug: "how-to-write-a-resume-that-passes-applicant-tracking-systems",
    title: "How to Write a Resume That Passes Applicant Tracking Systems",
    description: "Practical formatting and content tips so your resume actually gets seen by a human.",
    date: "2026-04-09",
    category: "Business",
    relatedTools: ["resume-builder", "ai-resume-review"],
    content: [
      "Most mid-size and large companies filter incoming resumes through an applicant tracking system (ATS) before a human ever sees them. These systems parse the text content of your resume and score it against the job description, which means a resume that looks great visually can still get filtered out if its underlying structure confuses the parser.",
      "The single most common mistake is relying on complex layouts — multi-column designs, text boxes, or tables — that look clean to a human eye but parse incorrectly or out of order when read by software expecting a linear document. A single-column layout with clear section headings (Experience, Education, Skills) is far more reliable, even if it looks slightly less creative.",
      "Keyword matching matters more than most people expect. If a job posting specifically mentions 'project management' or a particular software tool, using those exact terms somewhere in your resume — assuming they genuinely apply to your experience — meaningfully improves your match score, compared to a synonym or a more creative phrasing that a human reader might appreciate but a keyword-matching system won't credit.",
      "Once you have a draft, it's worth a second pass focused specifically on impact and clarity: replacing vague statements like 'responsible for managing a team' with specific, measurable outcomes like 'managed a team of 6, reducing project turnaround time by 30%.' This is exactly the kind of feedback an AI review pass can help catch quickly, on top of a standard proofread.",
    ],
  },
  {
    slug: "how-pdf-encryption-actually-works",
    title: "How PDF Encryption Actually Works, in Plain Language",
    description: "A non-technical explanation of what happens when you password-protect a PDF.",
    date: "2026-04-16",
    category: "Security",
    relatedTools: ["protect-pdf", "unlock-pdf"],
    content: [
      "When a PDF is encrypted, the actual content of the file — text, images, everything — is scrambled using a mathematical algorithm and a key derived from your password. Without the correct password, that scrambled data can't be turned back into a readable document; there's no 'back door' shortcut, which is exactly the point of encryption done correctly.",
      "Modern PDF encryption typically uses AES (Advanced Encryption Standard) with a 256-bit key, the same class of encryption used to protect online banking and government communications. The strength of this encryption isn't really the weak point in practice — a well-chosen 256-bit key is not something that can be brute-forced with any realistic amount of computing power. The actual weak point is almost always the password itself: a short, common, or guessable password can be tried repeatedly far faster than the underlying encryption could ever be broken directly.",
      "This is why a genuinely strong PDF password matters more than people often assume — using a long, unique passphrase rather than a short word or a reused password gives you the full benefit of the strong encryption underneath it, rather than undermining it with an easy entry point.",
    ],
  },
  {
    slug: "batch-processing-files-when-it-makes-sense",
    title: "Batch Processing Files: When It's Worth Doing All at Once",
    description: "How to think about processing many files at a time versus one at a time.",
    date: "2026-04-23",
    category: "Image",
    relatedTools: ["compress-image", "merge-pdf"],
    content: [
      "If you regularly find yourself repeating the exact same operation — compressing a folder of product photos, converting a batch of scanned receipts — doing it one file at a time is usually the slowest and most error-prone way to work, simply because it multiplies the chance of a manual mistake by however many files you have.",
      "The main thing to check before batch processing a large set of files is whether they actually need identical settings. Product photos from the same camera and lighting setup usually compress well with one shared quality setting; a mixed folder of scans, screenshots, and photos usually doesn't, since a screenshot with sharp text needs very different handling from a photo with smooth gradients. Grouping files by type before batch processing avoids the common failure mode of one setting looking great on some files and terrible on others.",
      "For anything irreversible — like overwriting originals — it's worth keeping a backup of the source files until you've spot-checked a few results from the batch. A setting that looks fine on the first file in a batch of fifty doesn't guarantee it'll look fine on all fifty, especially if the files aren't perfectly uniform.",
    ],
  },
  {
    slug: "understanding-pdf-page-numbers-and-headers",
    title: "Getting Page Numbers and Headers Right in a Long PDF",
    description: "Practical tips for numbering and labeling multi-section documents.",
    date: "2026-04-30",
    category: "PDF",
    relatedTools: ["add-page-numbers", "add-header-footer"],
    content: [
      "Long documents — reports, theses, compiled proposals — often need page numbers that don't simply start at 1 on the very first page. A common convention is to number a cover page and table of contents separately (often with roman numerals, or no visible number at all) and start the main numbering sequence from the first real content page.",
      "When a document is assembled by merging several separate PDFs together, it's important to add page numbers after merging, not before. Numbering each source file individually before merging means every file restarts at page 1, which produces a merged document with several separate '1, 2, 3...' sequences instead of one continuous count — confusing for anyone trying to reference 'page 42' in the final document.",
      "Headers and footers work well for repeating context that isn't about page position at all — a document title, a confidentiality notice, a client name — content that should appear consistently on every page without being part of the main content flow. Keeping this distinct from page numbering makes documents easier to update later, since you're not tangling static labels with a numbering sequence that shifts if pages are added or removed.",
    ],
  },
  {
    slug: "common-csv-file-problems-and-how-to-avoid-them",
    title: "Common CSV File Problems (and How to Avoid Them)",
    description: "Why CSV files break more often than they should, and how to keep your data clean.",
    date: "2026-05-07",
    category: "Data",
    relatedTools: ["csv-to-json", "json-to-csv"],
    content: [
      "CSV looks simple — comma-separated values — but that simplicity hides a surprising number of edge cases that trip up both humans and software. The most common one: any field that itself contains a comma needs to be wrapped in quotes, or the file structure breaks, since a raw comma inside a field is indistinguishable from a comma meant to separate columns.",
      "A related problem shows up with fields containing line breaks — a multi-line address or a comment field — which also need to be quoted, otherwise what looks like one row to a human reading the raw text actually gets parsed as multiple rows by software expecting one row per line.",
      "Encoding mismatches are another quiet source of corruption: a CSV saved with special characters (accented letters, currency symbols, emoji) in one encoding and opened assuming a different encoding will show garbled characters, even though the underlying data was never actually damaged — it's purely a mismatch in how the bytes are being interpreted.",
      "When converting CSV to JSON or back, these same edge cases matter just as much — a converter that naively splits on every comma without respecting quoted fields will silently produce wrong results on any row containing a comma inside a value.",
    ],
  },
  {
    slug: "what-a-hash-actually-is-explained-simply",
    title: "What a Hash Actually Is, Explained Without Jargon",
    description: "A plain-language explanation of hashing and why developers use it.",
    date: "2026-05-14",
    category: "Developer",
    relatedTools: ["hash-generator"],
    content: [
      "A hash is a fixed-length string of characters generated from any input — a word, a password, an entire file — using a mathematical function designed so that even a tiny change in the input produces a completely different hash output. Change a single letter in a sentence, and the resulting hash looks nothing like the hash of the original sentence, even though only one character changed.",
      "This property makes hashing useful for verification rather than encryption — the two are often confused, but they solve different problems. Encryption is meant to be reversed with the right key; a good hash function is specifically designed to be a one-way street, where you can't work backward from the hash to recover the original input. This is exactly why websites store a hash of your password rather than the password itself: if their database is ever breached, the attacker gets hashes, not usable passwords.",
      "Hashing is also how file integrity checks work: if you download a large file and the provider publishes its hash, you can hash your downloaded copy and compare the two values. If they match exactly, you can be confident the file wasn't corrupted or tampered with in transit.",
    ],
  },
  {
    slug: "why-file-size-limits-exist-and-how-to-work-around-them",
    title: "Why File Size Limits Exist (and How to Work Around Them)",
    description: "Understanding upload and email attachment limits, and the fastest way to fit under them.",
    date: "2026-05-21",
    category: "PDF",
    relatedTools: ["compress-pdf", "split-pdf"],
    content: [
      "Email providers, web forms, and file upload systems almost always enforce a maximum file size — commonly 10–25MB for email attachments, sometimes much smaller for web forms. These limits exist mainly to protect server storage and bandwidth costs, and to keep large attachments from clogging inboxes and slowing down mail delivery for everyone.",
      "When a file is too large, there are really two different strategies, and picking the right one depends on why the file is large. If the size comes from high-resolution images inside a PDF, compressing the PDF directly is the fastest fix and preserves the document as a single file. If the file is large simply because it has an enormous number of pages, compression alone may not get it under the limit, and splitting it into smaller multi-part files is often the more reliable solution.",
      "It's worth checking which limit you're actually up against before choosing a strategy: some services limit the size of a single file, while others limit total attachment size across multiple files in one message, which changes whether splitting a large file into parts actually helps.",
    ],
  },
];
